...
const SpotifyCallback = () => {
  ...
  useEffect(() => {
    // Use external script to handle Spotify callback
    if (window.SpotifyAuth) {
      const result = window.SpotifyAuth.handleSpotifyCallback();
      ...
      // Fallback to original logic if external script not loaded
      ...
      if (code) {
        ...
        fetch('http://15.207.204.90:5500/exchange-token', {
          ...
          .catch(error => {
            ...
            setError(error.message || 'Error exchanging code for token');
            setConnected(false);
          });
      }
    }
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-yellow-50 font-comic px-4 sm:px-0">
      <div className="rounded-2xl border-4 border-black bg-yellow-100 p-6 sm:p-10 shadow-xl comic-shadow flex flex-col items-center max-w-sm sm:max-w-md w-full">
        <ComicText fontSize={2.5} className="sm:text-3xl mb-3 sm:mb-4 text-center">
          {connected ? 'Spotify Connected!' : error ? 'Connection Failed' : 'Connecting to Spotify...'}
        </ComicText>
        <p className="font-comic text-base sm:text-lg mb-6 sm:mb-8 text-green-900 font-bold text-center">
          {connected ? 'You can now return to the app.' : error ? error : 'Please wait...'}
        </p>
        {connected && (
          <button
        ...
        {error && (
          <button
            className="px-6 sm:px-8 py-2.5 sm:py-3 bg-red-200 hover:bg-red-300 border-2 border-black rounded-full font-comic font-bold text-base sm:text-lg shadow comic-shadow transition"
            onClick={() => navigate('/')}
          >
            Try Again
          </button>
